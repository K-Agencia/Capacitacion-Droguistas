...
exports.get_data = async (req, res, next) => {
    ...
    const querySQL1 = `
    SELECT U.id_usuarios, U.correo, D.distribuidores, 
        ...
        CASE WHEN C7.id_capacitaciones IS NOT NULL THEN 'Visto' ELSE 'No' END AS producto7,
        CASE WHEN C8.id_capacitaciones IS NOT NULL THEN 'Visto' ELSE 'No' END AS producto8
    FROM users U
        JOIN distribuidores D ON D.id_distribuidores = U.distribuidor
        JOIN puntos_venta P ON P.id_puntos_venta = U.punto_venta
        JOIN zonas Z ON Z.id_zonas = P.id_zonas
        JOIN regiones R ON R.id_regiones = Z.id_regiones
        ...
        LEFT JOIN  capacitaciones C7 ON U.id_usuarios = C7.id_usuarios AND C7.id_productos = 7
        LEFT JOIN  capacitaciones C8 ON U.id_usuarios = C8.id_usuarios AND C8.id_productos = 8
    ...
    FROM
        ...
        capacitaciones C6 ON U.id_usuarios = C6.id_usuarios
            ...
            LEFT JOIN
        capacitaciones C7 ON U.id_usuarios = C7.id_usuarios
            AND C7.id_productos = 7
            LEFT JOIN
        capacitaciones C8 ON U.id_usuarios = C8.id_usuarios
            AND C8.id_productos = 8
            JOIN
