...
exports.excel = (req, res, next) => {
  ...
  const filename = `./src/uploads/Reporte - Capacitación Droguistas - ${fecha.getDate()}-${meses[fecha.getMonth()]
  ...
  ws.cell(2, 17).string("Cepillo Colgate Sensitive").style(azulBoldStyle);
  ws.cell(2, 18).string("Colgate Sensitive Pro Alivio Xtreme Temperatures").style(azulBoldStyle);
  ...
  rows.forEach((row, index) => {
    ...
    ws.cell(index + 3, 17)
      .string(row.producto7)
      .style(isVisto(row.producto7));
    ws.cell(index + 3, 18)
      .string(row.producto8)
      .style(isVisto(row.producto8));
  });
